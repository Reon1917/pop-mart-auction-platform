    ...
    >
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-4">
        <div className="flex items-center gap-3">
          <Link
            href="/"
            className="flex items-center justify-center h-10 w-10 rounded-md bg-gradient-to-br from-violet-600 to-fuchsia-500 text-white font-bold text-sm shadow-lg shadow-violet-500/25 hover:shadow-violet-500/40 transition-shadow"
          >
            PM
          </Link>
          <div>
            <div className="flex items-center gap-2">
              ...
              {badge && (
                <span className="px-2 py-0.5 rounded-md bg-violet-100 text-violet-700 text-[10px] font-bold uppercase tracking-wider">
                  {badge}
                </span>
              )}
            </div>
...
export function UserMenu({ name, role, onSignOut }: UserMenuProps) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <button
        onClick={onSignOut}
        className="rounded-md border border-zinc-200 px-3 py-2 text-sm font-medium text-zinc-600 transition hover:border-zinc-300 hover:text-zinc-900"
      >
        Sign out
      </button>
    </div>
